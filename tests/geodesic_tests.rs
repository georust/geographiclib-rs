...
fn test_gen_direct_vs_cpp() {
    ...
    // let delta_entries = Arc::new(Mutex::new(vec![RecordDeltaEntry::new(); 9]));
    ...
    test_basic("Geodesic_GenDirect", 17, |line_num, items| {
        ...
        if outmask & caps::LATITUDE != 0 {
            // record_delta(items[9], result.1, line_num, &mut entries[1]);
            // todo: reduce allowed difference to 0.0
        ...
        if outmask & caps::REDUCEDLENGTH != 0 {
            ...
            if !f64::is_nan(result.5) {
                // record_delta(items[13], result.5, line_num, &mut entries[5]);
        ...
        if outmask & caps::GEODESICSCALE != 0 {
            ...
            if !f64::is_nan(result.6) {
                // record_delta(items[14], result.6, line_num, &mut entries[6]);
            ...
            if !f64::is_nan(result.7) {
                // record_delta(items[15], result.7, line_num, &mut entries[7]);
        ...
        if outmask & caps::AREA != 0 {
            ...
            if !f64::is_infinite(result.8) {
                // record_delta(items[16], result.8, line_num, &mut entries[8]);
    ...
    //     println!("result.{} abs {:e} at line {}, rel {:e} at line {}, sign-change {}", i, entry.diff_abs, entry.line_abs, entry.diff_rel, entry.line_rel, entry.sign_change);
    // });
}
...
fn test_geodesic_vs_cpp() {
    ...
    test_basic("Geodesic_Geodesic", -1, |line_num, items| {
        ...
        // todo: remove flattening exception
        if g.f > 0.0 {
