...
class GeodesicLine(object):
    ...
    def __init__(
        ...
        *salp1* and *calp1* should not be supplied; they are part of the
        ...
        self.caps = caps | Geodesic.LATITUDE | Geodesic.AZIMUTH | Geodesic.LONG_UNROLL

        # Guard against underflow in salp0
        # the latitude of the first point in degrees (readonly)
        self.lat1 = Math.lat_fix(lat1)
        ...
        # real cbet1, sbet1
        sbet1, cbet1 = Math.sincosd(Math.ang_round(lat1))
        sbet1 *= self._f1
        ...
        self._csig1 = self._comg1 = (
            cbet1 * self.calp1 if sbet1 != 0 or self.calp1 != 0 else 1
        )
        # sig1 in (-pi, pi]
        ...
        if self.caps & Geodesic.CAP_C1:
            ...
            self._C1a = list(range(Geodesic.GEODESIC_ORDER + 1))
            Geodesic._C1f(eps, self._C1a)
            self._B11 = Geodesic._sin_cos_series(
    ...
    def _gen_position(self, arcmode, s12_a12, outmask):
        ...
        if not (arcmode or (self.caps & (Geodesic.OUT_MASK & Geodesic.DISTANCE_IN))):
        ...
        if arcmode:
            ...
            if abs(self.f) > 0.01:
                ...
                B12 = Geodesic._sin_cos_series(True, ssig2, csig2, self._C1a)
                serr = (1 + self._A1m1) * (
                    sig12 + (B12 - self._B11)
                ) - s12_a12 / self._b
        ...
        if outmask & Geodesic.LONGITUDE:
            ...
            omg12 = (
                ...
                * (
                    sig12
                    - (math.atan2(ssig2, csig2) - math.atan2(self._ssig1, self._csig1))
                    + (
            ...
            lam12 = omg12 + self._A3c * (
                sig12
                + (Geodesic._sin_cos_series(True, ssig2, csig2, self._C3a) - self._B31)
            )
        ...
        if outmask & (Geodesic.REDUCEDLENGTH | Geodesic.GEODESICSCALE):
            B22 = Geodesic._sin_cos_series(True, ssig2, csig2, self._C2a)
        ...
        if outmask & Geodesic.AREA:
            B42 = Geodesic._sin_cos_series(False, ssig2, csig2, self._C4a)
            # real salp12, calp12
            if self._calp0 == 0 or self._salp0 == 0:
                ...
                calp12 = (
                    Math.sq(self._salp0) + Math.sq(self._calp0) * self._csig1 * csig2
                )
    ...
    def Position(self, s12, outmask=GeodesicCapability.STANDARD):
        ...
        *lon1*, *azi1*, *lat2*, *lon2*, *azi2*, *s12*, *a12* entries are
        ...
        a12, lat2, lon2, azi2, s12, m12, M12, M21, S12 = self._gen_position(
            False, s12, outmask
        )
    ...
    def SetArc(self, a13):
        ...
        self.a13 = a13
        self.s13 = self._gen_position(True, self.a13, Geodesic.DISTANCE)[4]
