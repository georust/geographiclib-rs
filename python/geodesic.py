...
class Geodesic:
    ...
    nC4x_ = 21
    maxit1_ = 20
    maxit2_ = maxit1_ + Math.DIGITS + 10  # 83
    ...
    def _sin_cos_series(sinp, sinx, cosx, c):
        ...
        while n:  # while n--:
            ...
            y0 = ar * y1 - y0 + c[k]
        return (
            2 * sinx * cosx * y0 if sinp else cosx * (y0 - y1)  # sin(2 * x) * y0
        )  # cos(x) * (y0 - y1)
    ...
    def _C1f(eps, c):
        ...
        d = eps
        o = 0
        for l in range(1, Geodesic.GEODESIC_ORDER + 1):  # l is index of C1p[l]
            m = (Geodesic.GEODESIC_ORDER - l) // 2  # order of polynomial in eps^2
    ...
    def _C2f(eps, c):
        ...
        d = eps
        o = 0
        for l in range(1, Geodesic.GEODESIC_ORDER + 1):  # l is index of C2[l]
            m = (Geodesic.GEODESIC_ORDER - l) // 2  # order of polynomial in eps^2
            c[l] = d * Math.polyval(m, coeff, o, eps2) / coeff[o + m + 1]
            o += m + 2
            d *= eps
    ...
    def __init__(self, a, f):
        ...
        *f*) is not a finite positive quantity.
        ...
        self._c2 = (
            ...
            * (
                ...
                if self._e2 == 0
                    ...
                    if self._e2 > 0
                    else math.atan(math.sqrt(-self._e2))
                )
        ...
        self._etol2 = (
            0.1
            * Geodesic.tol2_
            / math.sqrt(max(0.001, abs(self.f)) * min(1.0, 1 - self.f / 2) / 2)
        )
        ...
        self._A3x = list(range(Geodesic.GEODESIC_ORDER))
    ...
    def _A3coeff(self):
        ...
        o = 0
        k = 0
        for j in range(Geodesic.GEODESIC_ORDER - 1, -1, -1):  # coeff of eps^j
            m = min(Geodesic.GEODESIC_ORDER - j - 1, j)  # order of polynomial in n
    ...
    def _C3coeff(self):
        ...
        o = 0
        k = 0
        for l in range(1, Geodesic.GEODESIC_ORDER):  # l is index of C3[l]
            for j in range(Geodesic.GEODESIC_ORDER - 1, l - 1, -1):  # coeff of eps^j
                m = min(Geodesic.GEODESIC_ORDER - j - 1, j)  # order of polynomial in n
    ...
    def _C4coeff(self):
        ...
        o = 0
        k = 0
        for l in range(Geodesic.GEODESIC_ORDER):  # l is index of C4[l]
            for j in range(Geodesic.GEODESIC_ORDER - 1, l - 1, -1):  # coeff of eps^j
                m = Geodesic.GEODESIC_ORDER - j - 1  # order of polynomial in n
    ...
    def _C3f(self, eps, c):
        ...
        o = 0
        for l in range(1, Geodesic.GEODESIC_ORDER):  # l is index of C3[l]
            m = Geodesic.GEODESIC_ORDER - l - 1  # order of polynomial in eps
            mult *= eps
    ...
    def _Lengths(
        ...
        if outmask & Geodesic.DISTANCE:
            B1 = Geodesic._sin_cos_series(
                True, ssig2, csig2, C1a
            ) - Geodesic._sin_cos_series(True, ssig1, csig1, C1a)
            # Missing a factor of _b
            s12b = A1 * (sig12 + B1)
            if outmask & (Geodesic.REDUCEDLENGTH | Geodesic.GEODESICSCALE):
                B2 = Geodesic._sin_cos_series(
                    True, ssig2, csig2, C2a
                ) - Geodesic._sin_cos_series(True, ssig1, csig1, C2a)
                J12 = m0x * sig12 + (A1 * B1 - A2 * B2)
        elif outmask & (Geodesic.REDUCEDLENGTH | Geodesic.GEODESICSCALE):
            # Assume here that nC1_ >= nC2_
            for l in range(1, Geodesic.GEODESIC_ORDER):
                C2a[l] = A1 * C1a[l] - A2 * C2a[l]
            J12 = m0x * sig12 + (
                Geodesic._sin_cos_series(True, ssig2, csig2, C2a)
                - Geodesic._sin_cos_series(True, ssig1, csig1, C2a)
            )
    ...
    def _InverseStart(
        ...
        if shortline and ssig12 < self._etol2:
            ...
            calp2 = sbet12 - cbet1 * sbet2 * (
                Math.sq(somg12) / (1 + comg12) if comg12 >= 0 else 1 - comg12
            )
            ...
            abs(self._n) >= 0.1
            or csig12 >= 0  # Skip astroid calc if too eccentric
            or ssig12 >= 6 * abs(self._n) * math.pi * Math.sq(cbet1)
        ):
            ...
            if self.f >= 0:  # In fact f == 0 does not get here
                ...
                betscale = (
                    sbet12a / x if x < -0.01 else -self.f * Math.sq(cbet1) * math.pi
                )
            ...
            if y > -Geodesic.tol1_ and x > -1 - Geodesic.xthresh_:
                ...
                omg12a = lamscale * (
                    -x * k / (1 + k) if self.f >= 0 else -y * (1 + k) / k
                )
    ...
    def _Lambda12(
        ...
        eps = k2 / (2 * (1 + math.sqrt(1 + k2)) + k2)
        self._C3f(eps, C3a)
        B312 = Geodesic._sin_cos_series(
            True, ssig2, csig2, C3a
        ) - Geodesic._sin_cos_series(True, ssig1, csig1, C3a)
    ...
    def _GenInverse(self, lat1, lon1, lat2, lon2, outmask):
        ...
        lon12, lon12s = Math.ang_diff(lon1, lon2)
        # Make longitude difference positive.
        lonsign = 1 if lon12 >= 0 else -1
        # If very close to being on the same half-meridian, then make it so.
        lon12 = lonsign * Math.ang_round(lon12)
        lon12s = Math.ang_round((180 - lon12) - lonsign * lon12s)
        ...
        lat1 = Math.ang_round(Math.lat_fix(lat1))
        lat2 = Math.ang_round(Math.lat_fix(lat2))
        ...
        sbet2, cbet2 = Math.sincosd(lat2)
        sbet2 *= self._f1
        # Ensure cbet2 = +epsilon at poles
        sbet2, cbet2 = Math.norm(sbet2, cbet2)
        cbet2 = max(Geodesic.tiny_, cbet2)
        ...
        C1a = list(range(Geodesic.GEODESIC_ORDER + 1))
        C2a = list(range(Geodesic.GEODESIC_ORDER + 1))
        C3a = list(range(Geodesic.GEODESIC_ORDER))
        ...
        if meridian:
            ...
            s12x, m12x, dummy, M12, M21 = self._Lengths(
                ...
                cbet1,
                cbet2,
                outmask | Geodesic.DISTANCE | Geodesic.REDUCEDLENGTH,
                C1a,
                C2a,
            )
        ...
        if (
            ...
            if sig12 >= 0:
                ...
                while numit < Geodesic.maxit2_:
                    ...
                    if v > 0 and (
                        numit > Geodesic.maxit1_ or calp1 / salp1 > calp1b / salp1b
                    ):
                        salp1b = salp1
                        calp1b = calp1
                    elif v < 0 and (
                        numit > Geodesic.maxit1_ or calp1 / salp1 < calp1a / salp1a
                    ):
                        salp1a = salp1
                        calp1a = calp1
                    ...
                    tripb = (
                        ...
                        or abs(salp1 - salp1b) + (calp1 - calp1b) < Geodesic.tolb_
                    )
        ...
        if outmask & Geodesic.AREA:
            ...
            if calp0 != 0 and salp0 != 0:
                ...
                ssig1, csig1 = Math.norm(ssig1, csig1)
                ssig2, csig2 = Math.norm(ssig2, csig2)
                C4a = list(range(Geodesic.GEODESIC_ORDER))
                self._C4f(eps, C4a)
                B41 = Geodesic._sin_cos_series(False, ssig1, csig1, C4a)
                B42 = Geodesic._sin_cos_series(False, ssig2, csig2, C4a)
                S12 = A4 * (B42 - B41)
            else:
            ...
            if (
                ...
                # omg12 < 3/4 * pi
                comg12 > -0.7071
                and sbet2 - sbet1 < 1.75  # Long difference not too big
    ...
    def _GenDirect(self, lat1, lon1, azi1, arcmode, s12_a12, outmask):
        ...
        if not arcmode:
            outmask |= Geodesic.DISTANCE_IN
        line = GeodesicLine(self, lat1, lon1, azi1, outmask)
        return line._gen_position(arcmode, s12_a12, outmask)
